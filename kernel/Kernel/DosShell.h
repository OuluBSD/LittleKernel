#ifndef _Kernel_DosShell_h_
#define _Kernel_DosShell_h_
...
#define DOS_MAX_FILENAME_LENGTH 128
#define DOS_MAX_DRIVE_LETTERS 26
#define DOS_SECTOR_SIZE 512
#define DOS_MAX_SECTORS_PER_READ 128
#define DOS_MAX_SECTORS_PER_WRITE 128
...
// DOS interrupt 21h function numbers
...
private:
    ...
    uint8* command_line;            // Command line buffer
    uint32 command_line_length;     // Length of command line
    ...
    uint8* device_drivers;          // Device drivers
    uint32 device_driver_count;     // Number of device drivers
    Spinlock dos_shell_lock;          // Lock for thread safety

public:
    DosShell();
    ~DosShell();
    
    // Initialize the DOS shell
    bool Initialize();
    ...
    int DosLandlockCreateRuleset(const struct landlock_ruleset_attr* attr, size_t size, __u32 flags);
    int DosLandlockAddRule(int ruleset_fd, enum landlock_rule_type rule_type, const void* rule_attr, __u32 flags);
    int DosLandlockRestrictSelf(int ruleset_fd, __u32 flags);
    ...
    int DosRseq(struct rseq* rseq, uint32 rseq_len, int flags, uint32 sig);
    int DosPidfdSendSignal(int pidfd, int sig, siginfo_t* info, unsigned int flags);
    int DosOpenTree(int dfd, const char* pathname, unsigned int flags);
    int DosMoveMount(int from_dfd, const char* from_pathname, int to_dfd, const char* to_pathname, unsigned int flags);
    int DosFsopen(const char* fs_name, unsigned int flags);
    int DosFsconfig(int fs_fd, unsigned int cmd, const char* key, const void* value, int aux);
    int DosFsmount(int fs_fd, unsigned int flags, unsigned int mount_attrs);
    int DosFspick(int dfd, const char* path, unsigned int flags);
    int DosPidfdOpen(pid_t pid, unsigned int flags);
    int DosClone3(struct clone_args* cl_args, size_t size);
    int DosCloseRange(unsigned int fd, unsigned int max_fd, unsigned int flags);
    int DosOpenat2(int dirfd, const char* pathname, struct open_how* how, size_t size);
    int DosPidfdGetfd(int pidfd, int targetfd, unsigned int flags);
    int DosFaccessat2(int dirfd, const char* pathname, int mode, int flags);
    int DosProcessMadvise(int pidfd, const struct iovec* iov, size_t iovcnt, int advice, unsigned long flags);
    int DosEpollPwait2(int epfd, struct epoll_event* events, int maxevents, const struct timespec* timeout,
                      const sigset_t* sigmask, size_t sigsetsize);
    int DosMountSetattr(int dfd, const char* path, unsigned int flags, struct mount_attr* uattr, size_t usize);
    int DosQuotactlFd(unsigned int fd, unsigned int cmd, int id, void* addr);
    int DosLandlockCreateRuleset(const struct landlock_ruleset_attr* attr, size_t size, __u32 flags);
    int DosLandlockAddRule(int ruleset_fd, enum landlock_rule_type rule_type, const void* rule_attr, __u32 flags);
    int DosLandlockRestrictSelf(int ruleset_fd, __u32 flags);
    int DosMemfdSecret(unsigned int flags);
    int DosProcessMrelease(int pidfd, unsigned int flags);
    ...
    int DosSetMempolicyHomeNode(unsigned long start, unsigned long len, unsigned long home_node, unsigned long flags);
...
// Global DOS shell instance
extern DosShell* g_dos_shell;

// Initialize the DOS shell
bool InitializeDosShell();
